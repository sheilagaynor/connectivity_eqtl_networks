...
for (ii in 1:length(tissue_list[,1])){
  ...
  print(tissue_id)
  for (split_category in c('test','train')){
    for (jj in 1:5){
  #Read in that tissues degrees for matrixeqtl, others
  eqtl_matrixeqtl <- readRDS(paste0(base.dir,"Out/Edges_split_matrixeqtl/Edges_split_matrixeqtl_",tissue_id,'_',jj,'_',split_category,".Rds"))
  ...
  eqtl_others <- readRDS(paste0(base.dir,"Out/Edges_split_others/",tissue_id,'/',"Edges_other_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  ...
  saveRDS(bh_deg[substr(as.character(bh_deg$node),1,3)=='chr',], file=paste0(base.dir,"Out/Degree_split_snp/Degree_BH_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  saveRDS(bh_deg[substr(as.character(bh_deg$node),1,4)=='ENSG'], file=paste0(base.dir,"Out/Degree_split_gene/Degree_BH_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  #Get others deg
  ...
  saveRDS(oth_deg[substr(as.character(oth_deg$node),1,3)=='chr',], file=paste0(base.dir,"Out/Degree_split_snp/Degree_Oth_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  saveRDS(oth_deg[substr(as.character(oth_deg$node),1,4)=='ENSG'], file=paste0(base.dir,"Out/Degree_split_gene/Degree_Oth_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  #Get pnull deg
  pnull_deg <- calc_deg_pnull_mat(eqtl_others)
  saveRDS(pnull_deg, file=paste0(base.dir,"Out/Degree_split_snp/Degree_Pnull_",tissue_id,'_',split_category,'_',jj,".Rds"))  
  saveRDS(pnull_deg, file=paste0(base.dir,"Out/Degree_split_gene/Degree_Pnull_",tissue_id,'_',split_category,'_',jj,".Rds"))  

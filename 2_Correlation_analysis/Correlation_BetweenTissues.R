...
combo_rows <- which(t(unlist(combn(length(tissues),2)))[,1] == arrayid)

#Read in the first tissue's degree output
tissue1_bh_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_BH_",tissues[arrayid],".Rds"))
setnames(tissue1_bh_snp, paste0(names(tissue1_bh_snp),'_test')); names(tissue1_bh_snp)[1] <- 'node'
tissue1_bh_gene <- readRDS(paste0(base.dir,"Out/Degree_gene/Degree_BH_",tissues[arrayid],".Rds"))
setnames(tissue1_bh_gene, paste0(names(tissue1_bh_gene),'_test')); names(tissue1_bh_gene)[1] <- 'node'
#Get others deg
tissue1_oth_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_Oth_",tissues[arrayid],".Rds"))
setnames(tissue1_oth_snp, paste0(names(tissue1_oth_snp),'_test')); names(tissue1_oth_snp)[1] <- 'node'
tissue1_oth_gene <- readRDS(paste0(base.dir,"Out/Degree_gene/Degree_Oth_",tissues[arrayid],".Rds"))
setnames(tissue1_oth_gene, paste0(names(tissue1_oth_gene),'_test')); names(tissue1_oth_gene)[1] <- 'node'
#Get pnull deg
tissue1_pnull_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_Pnull_",tissues[arrayid],".Rds"))
setnames(tissue1_pnull_snp, paste0(names(tissue1_pnull_snp),'_test')); names(tissue1_pnull_snp)[1] <- 'node'
tissue1_pnull_gene <- readRDS(file=paste0(base.dir,"Out/Degree_gene/Degree_Pnull_",tissues[arrayid],".Rds"))
...
for (chunk in 1:length(combo_rows)) {
  loop_array_id <- t(unlist(combn(length(tissues),2)))[combo_rows[chunk],2]
  #Read in the first tissue's degree output
  tissue2_bh_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_BH_",tissues[loop_array_id],".Rds"))
  setnames(tissue2_bh_snp, paste0(names(tissue2_bh_snp),'_train')); names(tissue2_bh_snp)[1] <- 'node'
  tissue2_bh_gene <- readRDS(paste0(base.dir,"Out/Degree_gene/Degree_BH_",tissues[loop_array_id],".Rds"))
  setnames(tissue2_bh_gene, paste0(names(tissue2_bh_gene),'_train')); names(tissue2_bh_gene)[1] <- 'node'
  #Get others deg
  tissue2_oth_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_Oth_",tissues[loop_array_id],".Rds"))
  setnames(tissue2_oth_snp, paste0(names(tissue2_oth_snp),'_train')); names(tissue2_oth_snp)[1] <- 'node'
  tissue2_oth_gene <- readRDS(paste0(base.dir,"Out/Degree_gene/Degree_Oth_",tissues[loop_array_id],".Rds"))
  setnames(tissue2_oth_gene, paste0(names(tissue2_oth_gene),'_train')); names(tissue2_oth_gene)[1] <- 'node'
  #Get pnull deg
  tissue2_pnull_snp <- readRDS(paste0(base.dir,"Out/Degree_snp/Degree_Pnull_",tissues[loop_array_id],".Rds"))
  setnames(tissue2_pnull_snp, paste0(names(tissue2_pnull_snp),'_train')); names(tissue2_pnull_snp)[1] <- 'node'
  tissue2_pnull_gene <- readRDS(file=paste0(base.dir,"Out/Degree_gene/Degree_Pnull_",tissues[loop_array_id],".Rds"))
  ...
  #Name the columns
  colnames(snp_deg_corr_loop) <- c(paste0( tissues[arrayid] , '.', tissues[t(unlist(combn(length(tissues),2)))[combo_rows[chunk],2]], '.either'),
                              paste0( tissues[arrayid] , '.', tissues[t(unlist(combn(length(tissues),2)))[combo_rows[chunk],2]], '.both') )
  colnames(gene_deg_corr_loop) <- c(paste0( tissues[arrayid] , '.', tissues[t(unlist(combn(length(tissues),2)))[combo_rows[chunk],2]], '.either'),
                              paste0( tissues[arrayid] , '.', tissues[t(unlist(combn(length(tissues),2)))[combo_rows[chunk],2]], '.both') )
  gene_deg_corr <- cbind(gene_deg_corr,gene_deg_corr_loop)
  snp_deg_corr <- cbind(snp_deg_corr,snp_deg_corr_loop)
  }

saveRDS(snp_deg_corr, paste0('/GTEX_V8/Out/Corr_between_snp/Degree_corr_between_',arrayid,'_snp.Rds'))
saveRDS(gene_deg_corr, paste0('/GTEX_V8/Out/Corr_between_gene/Degree_corr_between_',arrayid,'_gene.Rds'))

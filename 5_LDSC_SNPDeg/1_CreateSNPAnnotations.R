...
if (meas == 'bh'){
  ...
  degree_vals <- readRDS(paste0('/GTEX_V8/Out/Degree_snp/Degree_Oth_',tissue_id,'.Rds'))
  degree_vals$network_annot <- as.numeric(degree_vals$lfdr_ind_deg_all_0.05 >= 0)
  degree_vals$degree_annot <- as.numeric(degree_vals$lfdr_wgt_deg_all_0.05 >= quantile(degree_vals$lfdr_wgt_deg_all_0.05, 0.75, na.rm=T))
} else if (meas == "qval"){
  degree_vals <- readRDS(paste0('/GTEX_V8/Out/Degree_snp/Degree_Oth_',tissue_id,'.Rds'))
  degree_vals$network_annot <- as.numeric(degree_vals$qval_ind_deg_all_0.05 >= 0)
  degree_vals$degree_annot <- as.numeric(degree_vals$qval_wgt_deg_all_0.05 >= quantile(degree_vals$qval_wgt_deg_all_0.05, 0.75, na.rm=T))
}
...
#Make annotations for each chromosome
  for (chr in 1:22){
    ...
    write.table(baseld[baseld$SNP %in% bed_chr$SNP,],
                file=gzfile(paste0('/GTEX_V8/Data/LDSC_SNPDegree_Bin/',tissue_id,'/',meas,'_snp_baselineLD.',chr,'.annot.gz')),
                row.names = FALSE, sep="\t", quote = FALSE)
    write.table(bed_chr[,c('CHR','BP','SNP','CM','network_annot')],
                file=gzfile(paste0('/GTEX_V8/Data/LDSC_SNPDegree_Bin/',tissue_id,'/',meas,'_snp_network_annot.',chr,'.annot.gz')),
                row.names = FALSE, sep="\t", quote = FALSE)
    write.table(bed_chr[,c('CHR','BP','SNP','CM','degree_annot')],
                file=gzfile(paste0('/GTEX_V8/Data/LDSC_SNPDegree_Bin/',tissue_id,'/',meas,'_snp_degree_annot.',chr,'.annot.gz')),
                row.names = FALSE, sep="\t", quote = FALSE)
    #Prepare frq, bim/bed/fam
    snp_list <- fread(paste0('/GTEX_V8/Data/LDSC_SNPDegree_Bin/',tissue_id,'/',meas,'_snp_network_annot.',chr,'.annot.gz'))
    write.table(data.frame(snp_list$SNP),file=paste0("/GTEX_V8/Data/LDSC_SNPDegree_Bin/",tissue_id,'/',meas,"_snp_kept_snps.snplist"),row.names=FALSE,quote=FALSE,col.names=FALSE,sep="\t")
    frq <- fread(paste0('/ldsc_2020/1000G_Phase3_frq/1000G.EUR.QC.',chr,'.frq'))
    write.table(frq[frq$SNP %in% snp_list$SNP,],
                file=(paste0('/GTEX_V8/Data/LDSC_SNPDegree_Bin/',tissue_id,'/',meas,'_snp_1000G.EUR.QC.',chr,'.frq')),
                row.names = FALSE, sep="\t", quote = FALSE)
    ...
    system("module load plink/1.90-fasrc01; plink --bfile /ldsc_2020/1000G_EUR_Phase3_plink/1000G.EUR.QC.$chr \\
           --extract /GTEX_V8/Data/LDSC_SNPDegree_Bin/$tissue_id'/'$meas'_snp_kept_snps.snplist' \\
           --make-bed --out /GTEX_V8/Data/LDSC_SNPDegree_Bin/$tissue_id'/'$meas'_snp_1000G.EUR.QC.'$chr ")
  }
